...
import {
  readArtifactDraftSectionCitations,
...
function positive(value: unknown, label: string): number {
  if (!Number.isSafeInteger(value) || (value as number) < 1) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, `${label} is invalid`);
...
export async function readArtifactDraftInternal(
  ...
  if (artifact.artifact_id !== artifactRef.id || artifact.revision !== artifactRef.revision) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft identity is inconsistent");
  ...
  if (reservation === null || authorityRow === null || !objectRows.success || !Array.isArray(objectRows.results) || head === null) {
    fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft durable authority is incomplete");
  }
  ...
  if (headRevision < artifactRef.revision || head.artifact_id !== artifactRef.id) {
    fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft head is inconsistent");
  }
  ...
  if (currentBinding === null || currentArtifact === null || currentArtifact.status !== "DRAFT" ||
    ...
    fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "current draft head binding is inconsistent");
  }
  if (reservation.state !== "FINALIZED" || reservation.artifact_id !== artifactRef.id || reservation.artifact_revision !== artifactRef.revision ||
    ...
    fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft reservation binding is inconsistent");
  }
  ...
  } catch { fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft operation intent is invalid"); }
  if (intent.operation_kind !== "REPORT" || !canonicalJson(intent) || authorityRow.outbox_id === null || authorityRow.topic !== reservation.topic ||
    ...
    fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft operation authority is inconsistent");
  }
  const manifestRow = objectRows.results.find((row) => row.object_kind === "MANIFEST" && row.object_ref === "manifest");
  if (manifestRow === undefined || objectRows.results.filter((row) => row.object_kind === "MANIFEST").length !== 1) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft manifest row is missing");
  ...
  catch { fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft manifest residency is invalid"); }
  ...
  if (manifestSha !== manifestObject.sha256 || specSha !== parsedManifest.revision.spec_digest ||
      specSha !== artifact.spec_digest || specSha !== reservation.spec_digest || parsedManifest.revision.status !== "DRAFT" ||
  ...
  for (let ordinal = 0; ordinal < parsedManifest.revision.sections.length; ordinal += 1) {
    ...
    if (section === undefined) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft section is missing");
    if (!contracts.has(section.contract_id)) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft section contract is missing");
  ...
  if (objectRows.results.length !== expected.size) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft object set is incomplete");
  ...
  for (const expectedObject of expected.values()) {
    ...
    if (row === undefined) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft object mapping is incomplete");
...
>>>>>>> 48b6e67
  ...
  for (const entry of plannedStored) {
    if (entry === null || typeof entry !== "object" || Array.isArray(entry)) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft planned object is invalid");
    const value = entry as Record<string, unknown>;
    const ref = text(value.object_ref, "planned object ref");
    if (plannedRefs.has(ref)) fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft planned object refs are duplicated");
    plannedRefs.add(ref);
...
>>>>>>> 48b6e67
  if (citations) {
    ...
    const verificationObject = storedByRef.get(selectedSectionValue.verification_receipt_ref);
    const dependencyObject = storedByRef.get(parsedManifest.revision.dependency_manifest_ref);
    if (verificationObject?.bytes === undefined || dependencyObject?.bytes === undefined) {
      fail("ARTIFACT_DRAFT_READ_INTEGRITY", 409, "draft citation references are unavailable");
    }
    const citationInput: ArtifactDraftSectionCitationsContext = {
      artifact_ref: parsedManifest.revision.artifact_ref,
      evidence_freeze_ref: { id: artifact.evidence_freeze_id as string, revision: artifact.evidence_freeze_revision as number },
