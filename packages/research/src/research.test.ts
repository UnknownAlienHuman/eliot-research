...
import {
  createD1InvestigationLedgerStore, createInvestigationLedgerService, defaultResearchWorkflowPlan,
  type LedgerD1Database, type LedgerEvent, type LedgerHead,
} from "./index.js";
import { setup, baseInput, nextInput, seedHandles, invariant, codeOf, eventCount, headCount } from "./research-test-fixture.js";
...
describe("investigation ledger over actual D1 rows", () => {
  ...
  it("negatives reject unknown fields, malformed digests, duplicates, bounds, stale checkpoints and bad handles", async () => {
    ...
    expect(await codeOf(ctx.service.create({ ...input, goal: "g".repeat(262145) }))).toBe("LEDGER_INPUT_INVALID");
    await ctx.service.create(input);
    expect(await codeOf(ctx.service.create({ ...input, investigation_id: "inv-2", goal: "g".repeat(262145) }))).toBe("LEDGER_INPUT_INVALID");
