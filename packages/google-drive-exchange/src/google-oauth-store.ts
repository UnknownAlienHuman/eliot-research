...
export function createD1GoogleOAuthIntentStore(database: D1Database, config: GoogleOAuthConfiguration,
  ...
  const assertClaim = async (input: GoogleOAuthIntent, signal: AbortSignal) => {
    ...
    if (lifecycle !== undefined) {
      const reconnect = await readReconnect(intent.intent_id, signal, true);
      if (reconnect === null) {
        if (lifecycle !== "auto") return oauthFail("GOOGLE_OAUTH_CLAIM_CHANGED");
        const existing = await db.prepare(`SELECT connection_id FROM google_exchange_connection
          WHERE connection_id=?1 AND ${SCHEMA}`).bind(configuration.connection_id).first<Record<string, unknown>>()
          .catch(() => oauthFail("GOOGLE_OAUTH_STORE_UNAVAILABLE"));
        if (existing) return oauthFail("GOOGLE_OAUTH_CLAIM_CHANGED");
      }
    }
    fresh(intent, signal);
  };
  ...
  const reconnectCurrentSql = `EXISTS (SELECT 1 FROM google_oauth_reconnect_intent r
    JOIN google_exchange_connection c ON c.connection_id=r.connection_id
    WHERE r.intent_id=?3 AND r.connection_id=?8 AND c.principal_id=?4 AND c.oauth_client_id=?9
      AND c.google_subject=?10 AND c.google_email=?11
      AND c.credential_generation=r.expected_credential_generation
      AND c.credential_revision=r.expected_credential_revision
      AND c.oauth_publishing_status='In production')`;
  const claimLifecycleSql = lifecycle === undefined
    ? "AND NOT EXISTS(SELECT 1 FROM google_exchange_connection WHERE connection_id=?8)"
    : lifecycle === "auto"
      ? `AND (NOT EXISTS(SELECT 1 FROM google_exchange_connection WHERE connection_id=?8) OR ${reconnectCurrentSql})`
      : `AND ${reconnectCurrentSql}`;
    ...
    async claim(raw, attemptId, codeHash, signal) {
      ...
      try {
        await db.prepare(`UPDATE google_oauth_intent SET state='EXCHANGING',attempt_id=?1,code_sha256=?2 WHERE intent_id=?3
          ...
          AND ${SCHEMA} ${claimLifecycleSql}`)
          .bind(attemptId, codeHash, intent.intent_id, owner.principal_id, owner.session_generation, serialized,
            oauthClock(now), configuration.connection_id,
            ...(lifecycle === undefined ? [] : [configuration.oauth_client_id, configuration.google_subject, configuration.google_email])).run();
  ...
  async function readReconnect(intentId: string, signal: AbortSignal, requireCurrentFence = false): Promise<null | { readonly expected_generation: string; readonly expected_revision: number; readonly current_subject: string; readonly current_email: string }> {
    check(signal);
    const row = await db.prepare(`SELECT r.expected_credential_generation,r.expected_credential_revision,c.google_subject,c.google_email
      ...
      WHERE r.intent_id=?1 AND r.connection_id=?2 AND c.principal_id=?3 AND c.oauth_client_id=?4
        AND c.google_subject=?5 AND c.google_email=?6
        AND c.oauth_publishing_status='In production'
        ${requireCurrentFence ? "AND c.credential_generation=r.expected_credential_generation AND c.credential_revision=r.expected_credential_revision" : ""}
        AND ${SCHEMA}`)
      .bind(intentId, configuration.connection_id, owner.principal_id, configuration.oauth_client_id, configuration.google_subject, configuration.google_email)
