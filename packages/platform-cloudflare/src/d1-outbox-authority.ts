...
function decode(row: AuthorityRow): DecodedAuthority {
  ...
  try {
    ...
    fail("DELIVERY_INPUT_INVALID", "stored operation intent is malformed", false, cause);
  }
  assertDeliveryIdentifier(row.outbox_id, "outbox_id");
  if (typeof row.topic !== "string" || !TOPIC.test(row.topic)) {
    fail("DELIVERY_INPUT_INVALID", "outbox topic is invalid");
  }
...
const AUTHORITY_SELECT =
  "SELECT i.intent_id, i.revision, i.operation_kind, i.principal_ref, " +
  "i.idempotency_key, i.payload_ref, i.policy_decision_ref, " +
  "i.budget_reservation_ref, i.cancellation_ref, i.created_at, " +
...
async function readByIntentRef(
  database: D1Database,
  ...
  ).bind(intentRef.id, intentRef.revision).first<AuthorityRow>();
  return row === null ? null : decode(row);
}

async function readExistingAuthority(
  database: D1Database,
  intent: OperationIntent,
): Promise<DecodedAuthority | null> {
  const byIdempotency = await readByIdempotency(
    database,
    intent.operation_kind,
    intent.idempotency_key,
  );
  if (byIdempotency !== null) return byIdempotency;
  return readByIntentRef(database, intent.intent_ref);
}

function assertExact(
  existing: DecodedAuthority,
  ...
  if (
    ...
    fail(
      ...
      "intent_ref or idempotency identity is already bound to different intent/payload bytes",
    );
  }
}
...
export async function appendIntentWithOutbox(
  ...
  try {
    ...
    fail(
      "DELIVERY_INPUT_INVALID",
      "operation intent failed strict contract validation",
      false,
      error,
    );
  }
  if (!TOPIC.test(rawInput.topic)) fail("DELIVERY_INPUT_INVALID", "topic is invalid");
  if (!SHA256.test(rawInput.payload_sha256)) {
    fail("DELIVERY_INPUT_INVALID", "payload_sha256 is invalid");
  }
  ...
  try {
    ...
    fail(
      "DELIVERY_SETTLEMENT_UNCERTAIN",
      "intent/outbox atomic append failed",
      true,
      error,
    );
  }
