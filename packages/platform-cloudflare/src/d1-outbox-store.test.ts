  ...
  const database = {
    prepare(sql: string) {
      return {
        bind(...values: unknown[]) {
          return {
            async all<T>() { calls.push({ sql, values, method: "all" }); return take("all") as D1Result<T>; },
            async first<T>() { calls.push({ sql, values, method: "first" }); return take("first") as T | null; },
            async run<T>() { calls.push({ sql, values, method: "run" }); return take("run") as D1Result<T>; },
          };
        },
        async first<T>() { calls.push({ sql, values: [], method: "first" }); return take("first") as T | null; },
      };
...
describe("D1 outbox store", () => {
  ...
  it("returns no lease when another dispatcher wins the compare-and-swap", async () => {
    ...
    await expect(createD1OutboxStore(database).claimBatch({
      worker_id: "dispatcher-2", now_ms: 10_000, lease_ms: 10_000, limit: 10,
    })).resolves.toEqual([]);
  });
  ...
  it("reconciles a lost delivered acknowledgement only when the queue receipt is exact", async () => {
    const exact = fixture([
      ...
      { method: "first", value: { state: "SENT", queue_message_id: "outbox-1:1", lease_owner: null, lease_generation: 1, next_attempt_at: 0, last_error_code: null } },
    ]);
    ...
    const conflict = fixture([
      { method: "run", value: { success: true, meta: { changes: 0 } } },
      { method: "first", value: { state: "SENT", queue_message_id: "other", lease_owner: null, lease_generation: 1, next_attempt_at: 0, last_error_code: null } },
    ]);
    await expect(createD1OutboxStore(conflict.database).markDelivered(
      lease(),
      { queue_message_ref: "outbox-1:1", accepted_at_ms: 15_000 },
      15_000,
    )).rejects.toMatchObject({ code: "DELIVERY_SETTLEMENT_UNCERTAIN", retryable: true });
  });

  it("requires exact retry time and error code for idempotent settlement", async () => {
    const { database } = fixture([
      { method: "run", value: { success: true, meta: { changes: 0 } } },
      { method: "first", value: { state: "FAILED", queue_message_id: null, lease_owner: null, lease_generation: 1, next_attempt_at: 30_000, last_error_code: "QUEUE_SEND_FAILED" } },
    ]);
    await expect(createD1OutboxStore(database).markRetry(
      lease(), 30_000, "QUEUE_SEND_FAILED", 15_000,
    )).resolves.toBeUndefined();
  });

  it("reports bounded outbox health without treating missing rows as corruption", async () => {
    const { database } = fixture([
      { method: "first", value: { pending: 2, leased: 1, failed: 3, dead_lettered: 4, oldest_created_at: "1970-01-01T00:00:01.000Z" } },
    ]);
    await expect(readD1OutboxHealth(database, 11_000)).resolves.toEqual({
      pending: 2, leased: 1, failed: 3, dead_lettered: 4, oldest_unsent_age_ms: 10_000,
    });
  });
});
