...
import { createD1AiSearchGenerationRegistryStore } from
  "./ai-search-generation-registry-d1.js";
...
type WriteMode = "normal" | "throw-before-write" | "throw-after-write";

function cloneRow(row: StoredRow | null): StoredRow | null {
...
class D1RegistryFixture {
  ...
  public readonly calls: PreparedCall[] = [];
          ...
          bind(...values: unknown[]) {
            return {
              async first<T>(): Promise<T | null> {
                fixture.calls.push({ sql, values });
                return fixture.execute(sql, values) as T | null;
              },
            };
          },
        };
  ...
  private execute(
    ...
    if (sql.startsWith("SELECT ")) {
      return this.row?.namespace === values[0] ? cloneRow(this.row) : null;
    }
...
const EMPTY_REGISTRY: AiSearchGenerationRegistry = Object.freeze({
...
): AiSearchGenerationRegistryCasCommand {
  return {
    namespace: NAMESPACE,
...
function storedRow(
  value: AiSearchGenerationRegistrySnapshot,
): StoredRow {
  return {
...
function profile(): AiSearchInstanceProfile {
  return {
...
describe("D1 AI Search generation registry store", () => {
  ...
  it("rejects malformed commands before issuing SQL", async () => {
    ...
    await expect(
      store.compareAndSwap({
        ...command(first, third),
        extra: true,
      } as AiSearchGenerationRegistryCasCommand),
    ).rejects.toMatchObject({ code: "AI_SEARCH_REGISTRY_INPUT_INVALID" });
  ...
  it("rejects a missing D1 capability at composition time", () => {
    expect(() =>
      createD1AiSearchGenerationRegistryStore({} as D1Database),
    ).toThrowError(/database binding is invalid/u);
  });
});
