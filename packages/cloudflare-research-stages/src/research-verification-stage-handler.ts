...
import { digest, fail, WorkflowCheckpointError, type StageRequest, type WorkflowPrincipal, type WorkflowStageHandler } from "@eliotr/cloudflare-workflows";
...
function logVerificationFailure(phase: VerificationFailurePhase, error: unknown): void {
  console.error(JSON.stringify({
    event: "research_verification_failed",
    phase,
...
export function createResearchVerificationStageHandler(
  ...
  return async ({ request, principal, input_bytes, attempt_ref }) => {
    ...
    try {
      ...
      logVerificationFailure(phase, error);
      throw error;
    }
  };
}
