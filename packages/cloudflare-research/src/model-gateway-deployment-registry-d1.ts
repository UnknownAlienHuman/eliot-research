...
async function readCandidate(database: D1Database, candidateRef: string): Promise<StoredDynamicRouteCandidate | null> {
  ...
  return row === null ? null : decodeStoredDynamicRouteCandidate(row, "stored dynamic route candidate", "DYNAMIC_ROUTE_PROMOTION_FAILED");
}
...
async function readActive(database: D1Database, routeRef: string): Promise<{
  ...
  const raw = await database.prepare(`SELECT ${activeSelect} FROM dynamic_route_active_generation WHERE route_ref = ?1 LIMIT 1`).bind(routeRef).first<ActiveRowRaw>();
...
export function createD1DynamicRouteRegistry(
  ...
  return Object.freeze({
    async stageCandidate(rawCandidate: DynamicRouteCandidate, expectedSha256: string): Promise<DynamicRouteCandidateWriteReceipt> {
      const candidate = decodeDynamicRouteCandidate(rawCandidate, "DYNAMIC_ROUTE_REGISTRY_STAGE_FAILED");
      ...
      const candidateRef = dynamicRouteCandidateRefForSha(artifact.sha256);
      ...
      const sameVersion = await database.prepare(`SELECT ${candidateSelect} FROM dynamic_route_candidate WHERE route_ref = ?1 AND route_version = ?2 LIMIT 1`).bind(candidate.deployment.route_ref, candidate.deployment.route_version).first<StoredDynamicRouteCandidate["row"]>();
      if (sameVersion !== null) {
        const decoded = await decodeStoredDynamicRouteCandidate(sameVersion, "existing dynamic route version", "DYNAMIC_ROUTE_REGISTRY_STAGE_FAILED");
        if (decoded.sha256 !== artifact.sha256) stageFailure("route version is already bound to different candidate bytes");
      }
      let inserted: StoredDynamicRouteCandidate["row"] | null;
      try {
        inserted = await database.prepare("INSERT INTO dynamic_route_candidate(candidate_ref, route_ref, route_version, candidate_sha256, candidate_json, staged_at) VALUES (?1,?2,?3,?4,?5,?6) ON CONFLICT(candidate_ref) DO NOTHING RETURNING " + candidateSelect).bind(candidateRef, candidate.deployment.route_ref, candidate.deployment.route_version, artifact.sha256, artifact.json, timestamp(now(), "candidate staged_at", "DYNAMIC_ROUTE_REGISTRY_STAGE_FAILED")).first<StoredDynamicRouteCandidate["row"]>();
      } catch (cause) {
      ...
      const readback = await decodeStoredDynamicRouteCandidate(inserted, "staged dynamic route candidate", "DYNAMIC_ROUTE_REGISTRY_STAGE_FAILED");
...
export function createD1ModelGatewayDeploymentRegistry(
  ...
  const qualificationProofs = createD1DynamicRouteQualificationProofStore(database, { now });
  return Object.freeze({
    async resolve(routeRef: string): Promise<unknown | null> {
      ...
      const latest = await qualificationProofs.readLatest({
        route_ref: active.row.route_ref,
        route_version: active.row.route_version,
        candidate_ref: active.row.candidate_ref,
        candidate_sha256: active.row.candidate_sha256,
      });
      const qualificationTier = latest?.qualification.tier ?? active.candidate.candidate.qualification_tier;
      const qualificationExpiresAt = latest?.qualification.expires_at ?? active.candidate.candidate.qualification_expires_at;
      if (environment === "PRODUCTION" && qualificationTier !== "LIVE") failure("DYNAMIC_ROUTE_LIVE_GATE_REQUIRED", "production route resolution requires live qualification evidence");
