...
if (!["prepare", "dev", "smoke"].includes(command) || extra.length > 0) {
  ...
  try {
    ...
    try {
      ...
      try { await portGuard.release(); } catch (error) { releaseError = error; }
    }
    if (operationError !== undefined && releaseError !== undefined) {
      throw new AggregateError([operationError, releaseError], "local runtime failed and port guard cleanup failed");
    }
