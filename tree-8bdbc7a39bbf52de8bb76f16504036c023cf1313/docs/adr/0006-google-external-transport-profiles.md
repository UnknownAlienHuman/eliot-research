...
`gemini-mcp` is the selected Workspace profile. Gemini Spark and the official Google Workspace connector
own Google Drive, Docs and Sheets actions on the client side. ELIOT's MCP surface only supplies bounded
plans and candidate readback validation. It does not receive Google credentials, perform Google I/O, or
promote a caller-supplied receipt into canonical D1 state. Workspace selection therefore does not require
a Google Cloud project, custom OAuth client, Vertex configuration, or Gemini API key.
