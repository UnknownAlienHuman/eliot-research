...
const RESPONSE_REASON_PATTERN = "(?:FINGERPRINT_INVALID|LOG_READBACK_UNAVAILABLE|LOG_CORRELATION_INVALID|LOG_ID_MISSING|LOG_ID_INVALID|CONTENT_TYPE_INVALID|BODY_TOO_LARGE|BODY_JSON_INVALID|BODY_SHAPE_INVALID|MODEL_ID_INVALID|CACHE_INVALID|UNCLASSIFIED)";
const TRANSPORT_REASON_PATTERN = "(?:CANCELLED|DEADLINE_EXCEEDED|REDIRECTED|BODY_TOO_LARGE|BODY_READ_FAILED|NETWORK_CONNECTION_LOST|FETCH_TYPE_ERROR|FETCH_ERROR|ILLEGAL_INVOCATION|FETCH_NOT_SUPPORTED|UNCLASSIFIED)";
const QUALIFICATION_DETAIL_PATTERN = `(?:upstream HTTP [1-5][0-9]{2}|provider codes [0-9]{1,16}(?:,[0-9]{1,16}){0,7}|transport reason ${TRANSPORT_REASON_PATTERN}|response reason ${RESPONSE_REASON_PATTERN})`;
const SAFE_QUALIFICATION_FAILURE_TITLE = new RegExp(
  `^Document model qualification could not complete(?: \\(${QUALIFICATION_DETAIL_PATTERN}(?:; ${QUALIFICATION_DETAIL_PATTERN}){0,3}\\))?$`,
  "u",
);
