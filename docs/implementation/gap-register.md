...
Current owner priority (2026-09-13): finish the document-to-question-to-answer path with verified
...
Owner launch observations through 2026-09-14: Access, README upload and admitted reads, exact/lexical
search, a complete selected-document scan, and model-backed document-to-DRAFT runs have executed
...
The new immutable review records unresolved text and incomplete coverage; the original
revision remains unchanged. Its Recent work view shows the latest 20
owner-authorized events, with atomic `ARTIFACT_DRAFTED`, `RESEARCH_COMPLETED` and
`WIKI_PUBLISHED` producers from migrations `0060`–`0062`. The Wiki publication recorded at
`07:43:53.976Z` remains visible after its original scope expired through current owner
reauthorization. This is partial feed coverage: source admitted/updated/erasure producers and
the remaining feed scope are still open. Run `run-f1aab21e76884dab48b430cf2fedf64653c368615673b1eb`,
created at `2026-09-14T08:15:05.223Z`, reached `ENGINE_COMPLETED` at
`2026-09-14T08:17:10.002Z` (`current_revision=19`, `next_stage_index=18`); D1 recorded
`ARTIFACT_DRAFTED` and `RESEARCH_COMPLETED`. Both events appear in Recent work and the saved DRAFT
...
draft verification record and an owner citation response.
The record retains exact claim text, the performed audit's verdicts and support/counterevidence
handles. The PWA renders those verdicts and opens the corresponding saved citations. Legacy
records retain NOT_EXECUTED; EXECUTED means the check ran, not that every claim passed or that
the DRAFT is accepted. Invalid model audit observations are discarded as a whole and yield
`NOT_VERIFIABLE_IN_SCOPE`; they cannot erase the synthesis or become a passing claim assessment.
...
| P1 | Scope authority and explicit read grants are composed for owner metadata orientation and bounded FAST_SEARCH; S11 adds immutable delegation-revision scopes for service-token HTTP FAST_SEARCH and query-derived verify/open. S12 adds grantor-authored explicit-project report/section/citation reads; machine Research execution, machine-authored history, MCP readers and federation live qualification remain open | ER-10, ER-24, ER-30 | retain remote policy/grant/currentness receipts and complete non-metadata product paths |
...
4. Close the remaining named launch blockers: full RETRIEVAL, ERASURE and WIKI capability readiness remains open; the bounded owner from-run Wiki proposal/publication/reopen path is live on git-44ea8be. Federation still requires an independent mutually authenticated deployed peer and live range/cursor/replay/withdrawal receipts. Complete research.changes producers and selected gemini-mcp Workspace candidate byte admission/action readback. Use the selected Google transport without an unrelated Google Cloud project or Gemini CLI setup.
