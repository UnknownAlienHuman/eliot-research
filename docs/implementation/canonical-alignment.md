...
|---|---|
| Product §§1.2, 12.3–12.12; ADR-0003/0006: explicit Google transport profile | The selected `GOOGLE_EXTERNAL_TRANSPORT=gemini-mcp` profile uses client-side Workspace actions and candidate-only ELIOT readback; it does not require a Google Cloud project or custom OAuth client. The separate `drive-exchange` profile retains Drive's unfinished OAuth/REST/cursor/freeze/reconciliation/publication gates and one ChatGPT write transport. Gemini cannot replace that profile and Drive Exchange cannot auto-activate under Workspace. |
...
- **#95 / ER-18/19/20:** continue Drive OAuth/durable-cursor/Doc-publication adapters, one state family at a time. Five
  ...
  The encrypted D1 credential/refresh boundary and internal initial OAuth admission now exist
  (`drive-credentials.md`, `drive-oauth-admission.md`); owner HTTP/PWA integration, operator configuration
  admission and exchange provisioning remain missing. These components neither complete the required exchange nor replace its missing canonical admission loop. The optional
