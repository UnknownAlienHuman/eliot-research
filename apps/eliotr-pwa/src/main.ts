...
function render(health: SystemHealth | null): void {
  app.innerHTML = `
    <header class="topbar">
      <div><strong>Eliot Research</strong><span class="generation">${health?.deployment_generation ?? "unknown generation"}</span></div>
      ${healthBadge(health)}
    </header>
    <main class="workspace">
      ...
      <section class="panel panel--investigation">
        ...
        <div class="empty-state">
          ...
          <code>${health?.blocking_reason_codes.join(", ") || "No schema blockers reported"}</code>
        </div>
      </section>
      <aside class="panel panel--evidence">
        <h2>Evidence</h2>
        <dl>
          <dt>Core schema</dt><dd>${health?.core_schema_generation ?? "not applied"}</dd>
          <dt>Search schema</dt><dd>${health?.search_schema_generation ?? "not applied"}</dd>
...
void getSystemHealth().then(render).catch(() => render({
  ready: false, deployment_generation: "unreachable", core_schema_generation: null,
  search_schema_generation: null, blocking_reason_codes: ["HEALTH_ENDPOINT_UNREACHABLE"], checked_at: new Date().toISOString(),
}));
