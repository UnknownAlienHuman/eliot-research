...
export async function fixture(handlerGeneration = "retrieve-branches-v1", options: { readonly content_markdown?: string; readonly query?: string; readonly inquiry_protocol_ref?: VersionedRef } = {}): Promise<Fixture> {
  ...
  await importAndProject(world, options);
  ...
  return { world, db, bucket, scope, navigation, ledger: ledgerStore, stage0, stage0_receipt: stage0Receipt, executor };
}

export async function prepareRetrieveStage(f: Fixture): Promise<{
...
export async function rowCounts(db: D1Database): Promise<{ readonly snapshots: number; readonly grants: number; readonly profiles: number; readonly results: number; readonly traces: number }> {
