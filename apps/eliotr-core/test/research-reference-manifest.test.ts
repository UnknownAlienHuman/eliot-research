...
describe("research reference manifest over real D1/R2", () => {
  ...
  it("builds a deduplicated-source manifest from exact evidence and replays immutable D1/R2 bytes", async () => {
    ...
    const originalGrant = await db.prepare(
      "SELECT allowed_use_json FROM scope_access_grant WHERE authorization_receipt_ref = ?1",
    ).bind(held.authorization_receipt_ref).first<{ readonly allowed_use_json: string }>();
    ...
    await db.prepare("UPDATE scope_access_grant SET allowed_use_json = '[]' WHERE authorization_receipt_ref = ?1").bind(held.authorization_receipt_ref).run();
    ...
    await db.prepare("UPDATE scope_access_grant SET allowed_use_json = ?1 WHERE authorization_receipt_ref = ?2").bind(originalGrant.allowed_use_json, held.authorization_receipt_ref).run();
