...
} from "./federation-d1-authority.js";
import {
...
import type { FederationAuthorityBinding } from "./federation-service.js";
...
describe("ER-41 D1 federation job authority", () => {
  ...
  it("rejects rebinding an existing cancellation to another reason", async () => {
    const fixture = database([
      await jobRow({ cancellationReason: "first reason" }),
    ]);
    await expect(createD1FederationJobAuthority(fixture.db).cancel(
      binding(),
      "exchange-1",
      "idempotency-1",
      "second reason",
    ...
    expect(fixture.calls.some((call) => call.method === "run")).toBe(false);
  });
});
