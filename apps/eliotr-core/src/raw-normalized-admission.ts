...
import { BundleAdmissionReceiptSchema } from "@eliotr/contracts";
...
function receipt(value: unknown): RawNormalizedAdmissionResult["admission_receipt"] {
  ...
  const parsed = json<unknown>(value, "admission receipt");
  try { return BundleAdmissionReceiptSchema.parse(parsed); }
  catch (cause) { throw new RawNormalizedAdmissionError("RAW_NORMALIZED_STATE_INVALID", 503, "admission receipt is malformed", true, cause); }
}
