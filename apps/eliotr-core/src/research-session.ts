...
export class ResearchSession extends DurableObject<Env> {
  ...
  private async cancel(request: Request, sid: string): Promise<Response> {
    ...
    if (caller.credential_generation !== stored.credential_generation ||
        caller.deployment_generation !== stored.deployment_generation) {
    ...
    const before = await store.readRunStatus(stored.operation_id, caller);
    ...
    const cancellationReceipt = await store.cancel(stored.operation_id, caller);
    const after = await store.readRunStatus(stored.operation_id, caller);
