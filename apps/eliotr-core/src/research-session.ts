...
export function parseResearchQueryRequest(raw: unknown): QueryRequest { if (typeof raw !== "object" || raw === null || Array.isArray(raw)) fail("RESEARCH_INPUT_INVALID", "query request must be an object"); const r = raw as Record<string, unknown>; exactKeys(r); if (r.product !== "ORIENT" || r.evidence_grade !== "E0" || r.budget_ref !== ORIENTATION_PROFILE) fail("RESEARCH_PROFILE_UNSUPPORTED", "research.query supports only the ORIENT metadata profile", 422); return { query: checkQuery(r.query), product: "ORIENT", scope_expression: checkScope(r.scope_expression), literals: [], evidence_grade: "E0", budget_ref: ORIENTATION_PROFILE, max_results: checkLiteralsMax(r) }; }
...
export function createResearchRunService(env: Env): { run(context: AuthenticatedRequestContext, request: QueryRequest): Promise<{ investigation_ref: VersionedRef; workflow_instance_id: string }> } {
  return {
    async run(context, raw) {
      ...
      await db.prepare("INSERT OR IGNORE INTO investigation_current_policy (policy_generation, policy_authority_ref, state, created_at) VALUES (?1,?2,'ACTIVE',?3)").bind(POLICY_GEN, snapshotRow.policy_authority_ref, now).run().catch(mapLedger);
