//! Dependency-free SHA-256 for deterministic canonical-body identifiers.
//!
//! The implementation follows FIPS 180-4 and performs no I/O, allocation, clock, environment or
//! randomness access. The message length is encoded modulo 2^64 as required by SHA-256.
...
/// Computes the SHA-256 digest of exact input bytes.
#[must_use]
pub fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    if length_offset == 120 {
        ...
        second.copy_from_slice(&tail[64..]);
        compress(&mut state, &second);
    }

    let mut digest = [0_u8; 32];
    let (outputs, output_remainder) = digest.as_chunks_mut::<4>();
...
fn compress(state: &mut [u32; 8], block: &[u8; 64]) {
    ...
    let (words, word_remainder) = block.as_chunks::<4>();
